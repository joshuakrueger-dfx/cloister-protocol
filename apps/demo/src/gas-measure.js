...
async function main() {
  ...
  const shield = await buildTransaction({
    tree, chainId: 31337, poolAddress: await pool.getAddress(), inputs: [], outputs: [{ note: new Note({ amount: 1000n, pubKey: alice.publicKey }), encPubKey: alice.address().encPubKey }],
  ...
  const pay = await buildTransaction({
    tree, chainId: 31337, poolAddress: await pool.getAddress(), inputs: [{ note: n.note, privateKey: alice.privateKey, index: n.index }],
    outputs: [
