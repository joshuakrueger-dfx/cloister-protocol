...
async function deployPool(tokenAddr, verifierAddr, guardian) {
  ...
  return Pool.deploy(LEVELS, LANES, INITIAL_ROOT, verifierAddr, tokenAddr, guardian);
}
...
describe("ShieldedPool — security hardening", () => {
  ...
  describe("constructor validation", () => {
    it("rejects zero verifier / token, bad levels, oversized index space, bad initial root", async () => {
      ...
      await expect(Pool.deploy(LEVELS, LANES, INITIAL_ROOT, ethers.ZeroAddress, t, owner)).to.be.revertedWith("verifier");
      await expect(Pool.deploy(LEVELS, LANES, INITIAL_ROOT, v, ethers.ZeroAddress, owner)).to.be.revertedWith("token");
      await expect(Pool.deploy(0, LANES, INITIAL_ROOT, v, t, owner)).to.be.revertedWith("levels");
      await expect(Pool.deploy(33, LANES, INITIAL_ROOT, v, t, owner)).to.be.revertedWith("levels");
      // numLanes << levels must fit uint32: 2^13 << 20 = 2^33 > uint32 → revert
      await expect(Pool.deploy(20, 8192, INITIAL_ROOT, v, t, owner)).to.be.revertedWith("index space");
      const FIELD = 21888242871839275222246405745257275088548364400416034343698204186575808495617n;
      await expect(Pool.deploy(LEVELS, LANES, FIELD, v, t, owner)).to.be.revertedWith("initialRoot");
    });
  });
  ...
  describe("ERC20 safety", () => {
    it("works with a no-return (USDT-style) token via SafeERC20", async () => {
      ...
      await expect(
        pool.transact(PROOF, INITIAL_ROOT, 22222n, [1n, 2n], [3n, 4n], extData(ethers.ZeroAddress, 1000n, ethers.ZeroAddress, 0n)),
      ).to.not.be.reverted;
    ...
    it("rejects fee-on-transfer tokens on deposit (no under-collateralisation)", async () => {
      ...
      await expect(
        pool.transact(PROOF, INITIAL_ROOT, 22222n, [1n, 2n], [3n, 4n], extData(ethers.ZeroAddress, ethers.parseEther("1000"), ethers.ZeroAddress, 0n)),
  ...
  describe("duplicate nullifier", () => {
    it("rejects a tx with both input nullifiers equal", async () => {
      ...
      const pool = await deployPool(await tok.getAddress(), await verifier.getAddress(), owner.address);
      await expect(
        pool.transact(PROOF, INITIAL_ROOT, 22222n, [7n, 7n], [3n, 4n], extData(ethers.ZeroAddress, 0n, ethers.ZeroAddress, 0n)),
  ...
  describe("reentrancy", () => {
    it("blocks a hook-token from re-entering transact and draining the pool", async () => {
      ...
      await pool.transact(PROOF, INITIAL_ROOT, 100n, [1n, 2n], [3n, 4n], extData(ethers.ZeroAddress, ethers.parseEther("1000"), ethers.ZeroAddress, 0n));
      ...
      const reentryPayload = pool.interface.encodeFunctionData("transact", [
        PROOF, 100n, 200n, [10n, 11n], [12n, 13n], extData(attacker.address, -ethers.parseEther("500"), ethers.ZeroAddress, 0n),
      ]);
      await tok.setAttack(poolAddr, reentryPayload);

      // Outer withdraw of 500 → triggers the hook → re-entry must be blocked by the guard.
      await pool.transact(PROOF, 100n, 200n, [20n, 21n], [22n, 23n], extData(attacker.address, -ethers.parseEther("500"), ethers.ZeroAddress, 0n));
  ...
  describe("guardian deposit pause", () => {
    it("guardian can pause deposits but never blocks withdrawals", async () => {
      ...
      // deposit blocked
      await expect(
        pool.transact(PROOF, 100n, 200n, [30n, 31n], [32n, 33n], extData(ethers.ZeroAddress, ethers.parseEther("10"), ethers.ZeroAddress, 0n)),
      ).to.be.revertedWith("deposits paused");

      // withdrawal STILL works while paused (funds never frozen)
      await expect(
        pool.transact(PROOF, 100n, 200n, [40n, 41n], [42n, 43n], extData(attacker.address, -ethers.parseEther("100"), ethers.ZeroAddress, 0n)),
      ).to.not.be.reverted;
