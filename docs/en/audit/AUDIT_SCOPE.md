...
|---|---|---|---|
| **P1-9** | `kyc.js` sanctions screening remains a **hardcoded sample pre-screen** (`SANCTIONS_NAMES`). | **Critical (compliance)** | **Partially mitigated** — the sample endpoint never mints an ASP credential; ASP funding requires server-side DFX KYC attestation bound to the owner key. Full-list sanctions coverage, fuzzy matching and legal/provider sign-off remain open. |
| **P1-10** | `/v1/shielded/submit` caller-supplied `associationRoot` could be auto-published, defeating the compliance gate. | **Critical (compliance)** | **Root bypass fixed** — caller roots must already be known; proof/range validation, CORS allowlist and rate limits added. **Residual:** quote→commitment binding and durable authenticated settlement remain open. |
